...
3. **Done-criteria** — *decidable*: mechanically checkable wherever possible (exact test command, invariant, expected diff scope), otherwise judgeable from evidence by a Gate 2 verifier — then state what evidence would settle it. UI units additionally get a reachability criterion (component imported by a route-reachable file) and a post-merge runtime-mount check. If you can't state a decidable done-criterion either way, the unit is under-specified — re-decompose.
