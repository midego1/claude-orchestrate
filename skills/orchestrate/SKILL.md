...
3. **Delegate execution to a foreman.** For any plan with more than ~3 units, hand the full dispatch plan to an **opus foreman** sub-agent that runs the loop: dispatches workers, runs gates, triages failures, manages retries within budget. You re-enter only for capability escalations past the foreman's authority, plan changes, and final integration. For small plans (≤3 units), dispatch directly yourself. Sub-agents run in parallel wherever units don't depend on each other; serialize only true dependencies. **Parallelism caveat:** parallel dispatch applies to read-only workers; workers that mutate files in the same working tree must be serialized or given worktree isolation — they fight over lockfiles, build caches, and dev-server ports.
4. **Verify tiered** — never read raw sub-agent output yourself as the first check:
   - **Gate 1 (mechanical, ~free):** done-criteria must be machine-checkable wherever possible — a passing test, a clean build, a grep-checkable invariant, a diff limited to declared files. Run these as bash commands.
   ...
   - **Gate 3 (you):** only gate-passed, foreman-summarized output reaches you. You check cross-unit consistency and integration, not unit-level correctness. Gate results arrive as one line each; evidence is attached only on FAIL.
   - Never trust a sub-agent's self-report of success. A claim of success without gate evidence is a FAIL.
5. **Triage failures before escalating** — most failures are not capability failures:
   ...
   - **Capability failure** (spec was correct and complete, model genuinely couldn't do it) → escalate one tier up (effort first, then model), including the failed attempt and the failure reason in the new dispatch.
...
- Announce the routing plan (units → model/depth) before dispatching, in one compact table. No prose justification per unit.
- Default distribution for a typical feature: ~60% of dispatches T0/T1, ~35% T2, ≤5% T3/max. If your plan is heavier than that, re-decompose — you're under-specifying units.
