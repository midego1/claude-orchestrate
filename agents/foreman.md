...
1. **Dispatch** the worker exactly as the plan specifies (model, effort/depth, dispatch contract). Run independent units in parallel; serialize only true dependencies. Workers that mutate files in the same working tree must be serialized or given worktree isolation — they fight over lockfiles, build caches, and dev-server ports.
2. **Gate 1 — mechanical checks (bash):** run every machine-checkable done-criterion yourself — the exact test command, the build, the grep-checkable invariant, the diff-scope check. Free and decisive; always first.
...
Append every escalated or surfaced unit to `.claude/escalation-ledger.md`: unit description, initial tier, failure type (spec/env/capability), final tier, outcome. If the file doesn't exist, create it with the header row `unit | initial tier | failure type | final tier | outcome`.
