...
- **Verifiability gap** (the failure mode structurally cannot be exercised by the repo's test infrastructure) → do NOT escalate; a stronger model buys another equally unverifiable attempt. Reduce the unit to the verifiable subset, ship that, and surface the remainder as a scoped follow-up naming the missing test infra. Escalation is for capability gaps, not verifiability gaps.
